...
export default function EncounterSidebar({
  ...
  return (
    <div className="p-4 space-y-3">
      ...
      {isGM && (
        <Button size="sm" variant="outline" className="h-7 text-xs gap-1 w-full" onClick={() => createEncounterMutation.mutate()}>
            ...
            <div className="space-y-1">
              {encounters.map((enc) => (
                <div key={enc.id} className={cn(
                  ...
                  enc.id === activeEncounter?.id ? 'bg-primary/10 border-primary/40' : 'bg-card border-border/30'
                )}>
                  {renamingId === enc.id ? (
                    <>
