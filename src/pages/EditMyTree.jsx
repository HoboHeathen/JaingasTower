...
export default function EditMyTree() {
  ...
  const { data: tree, isLoading } = useQuery({
    queryKey: ['user-skill-tree', treeId],
    queryFn: () => base44.entities.UserSkillTree.filter({ id: treeId }),
    select: (data) => data[0],
    enabled: !!treeId,
  });

  const updateNodesMutation = useMutation({
    mutationFn: (nodes) => base44.entities.UserSkillTree.update(treeId, { nodes }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['user-skill-tree', treeId] });
      queryClient.invalidateQueries({ queryKey: ['user-skill-trees'] });
      toast.success('Saved');
    },
  });

  const updateTreeMutation = useMutation({
    mutationFn: (data) => base44.entities.UserSkillTree.update(treeId, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['user-skill-tree', treeId] });
      queryClient.invalidateQueries({ queryKey: ['user-skill-trees'] });
